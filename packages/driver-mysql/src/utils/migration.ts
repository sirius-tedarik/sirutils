...
export const migrationActions = createActions(
  async (context: Sirutils.DriverMysql.Context): Promise<Sirutils.DriverMysql.MigrationApi> => {
    ...
    return {
      migration: (
        ...
        return [
          ...
          wrap(
            async () => await rawUp(),
            `${driverMysqlTags.migration}#${name}.${version}.up` as Sirutils.ErrorValues
          ),
          wrap(
            async () => await rawDown(),
            `${driverMysqlTags.migration}#${name}.${version}.down` as Sirutils.ErrorValues
          ),
        ]
      },
          ...
          async (
            ...
            if (retryCount >= 2) {
              return ProjectError.create(
                `${driverMysqlTags.migration}#up.loop` as Sirutils.ErrorValues,
                'failed'
              )
                .appendData(error)
                .throw()
      ...
      down: async (migrations, targetVersion) => {
        ...
        const loop = wrap(
          async (
            ...
            if (retryCount >= 2) {
              return ProjectError.create(
                `${driverMysqlTags.migration}#down.loop` as Sirutils.ErrorValues,
                'failed'
              )
                .appendData(error)
                .throw()
